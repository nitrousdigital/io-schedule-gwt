...
public class SandBoxSelectionView extends Composite implements ToolbarEnabledWidget {
	...
	public SandBoxSelectionView(int width, int height) {
		...
		for (Sandbox sandbox: Sandbox.values()) {
			...
			Label label = new Label(sandbox.toString());
			label.setStyleName("sandboxSelectionRowText");
			label.setWidth((width - 40)+"px");
			final Sandbox clicked = sandbox;
			label.addClickHandler(new ClickHandler(){
				public void onClick(ClickEvent event) {
					if (controller != null) {
						controller.showSandbox(clicked);
					}
				}
			});
			row.add(label);
	...
	private Image getImage(Sandbox sandbox) {
		switch (sandbox) {
